...
def device_data(mac):
    ...
    """
    device = json.loads(redis_db.hget(mac, "data"))
    ...
    for app_trf in device_trf:
        if device_last_seen <= app_trf["last_seen"]:
            device_last_seen = app_trf["last_seen"]
        device_up_rate += app_trf["rate_up"]
        device_dn_rate += app_trf["rate_dn"]
